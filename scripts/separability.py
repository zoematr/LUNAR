...
def silhouette(X, y):
    """Mean silhouette over points for 2 labels (CASAL H.4). Euclidean."""
    sq = (X ** 2).sum(1)
...
def main():
    ...
    for l in range(num_layers):
        ...
        sil_full.append(silhouette(X, y))
        sil_2d.append(silhouette(pca(X, 2), y))
    ...
    json.dump({"model_family": args.model_family,
               ...
               "silhouette_full": [round(s, 4) for s in sil_full],
               "silhouette_pca2": [round(s, 4) for s in sil_2d]},
              open(save_dir / "silhouette.json", "w"), indent=2)

    print("\nper-layer silhouette (forget vs retain): higher = more separable")
    print(f"{'layer':>5} {'full-dim':>9} {'pca-2d':>8}")
    for l in range(num_layers):
        print(f"{l:5d} {sil_full[l]:9.3f} {sil_2d[l]:8.3f}")
    bl = int(np.argmax(sil_2d))
    print(f"\nmost separable layer (pca-2d): {bl}  (silhouette {sil_2d[bl]:.3f})")
